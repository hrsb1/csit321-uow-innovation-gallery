...
//Description: TagTable component for managing tags in the admin panel
"use client"
...
export const TagTable: React.FC = () => {
    ...
    const [data, setData] = useState<{ TagName: string; TagId: string }[]>([]);
    ...
    const feachTag = React.useCallback(async(token: string| null) => {
        const { data: tags, nextToken: newNextToken, errors } = await client.models.Tags.list({nextToken: token, limit: 12});
        if (!errors) {
    ...
    return (
        <div className="flex flex-col">
            <div className='flex justify-end items-center mb-4 gap-4 pr-[25px] pt-4'>
                ...
                onClick={() => setIsAddModalOpen(true)} >Add Tag</Button>
                <Button className ="custom-btn"
                ...
                <table className="min-w-full border-collapse border border-gray-600">
                    <thead>
                        <tr className="bg-gray-500 text-white">
                            <th className="w-[60%] border border-gray-600 px-4 py-2 text-center">Tag Name</th>
                            <th className="w-[40%] border border-gray-600 px-4 py-2 text-center">Edit Tag</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((tags, index) => (
                            <tr key={index} className={`text-center ${index % 2 === 0 ? 'bg-gray-300' : 'bg-gray-100'}`}>
                                <td className="w-[60%] border border-gray-600 px-4 py-2">{tags.TagName}</td>
                                <td className="w-[40%] border border-gray-600 px-4 py-2">
                                    <Button onClick={() => {
                                        formEdit.setFieldsValue({ tagName: tags.TagName});
            ...
            <Modal
                ...
                <Form
                    ...
                    onFinish={async (values) => {
                        ...
                        try {
                            if (data.some(tags => tags.TagName === tagName)) {
                                alert("Tag already exists");
                                return;
                            }
                            await client.models.Tags.create({
                                tagName
                            }, { authMode: "userPool" });
                            feachTag(pageTokens[currentPageIndex]);
                            form.resetFields();
                                ...
                                Papa.parse(text, {
                                    ...
                                    complete: async (results) => {
                                        ...
                                        for (const row of datacsv) {
                                            if (!data.some(tags => tags.TagName === row.tagName)) {
                                                try {
                                                    await client.models.Tags.create({
                                                        tagName: row.tagName.toLocaleLowerCase(),
                                                    }, { authMode: "userPool" });
                                                } catch (error) {
            ...
            >
                <Form
                    ...
                    onFinish={async (values) => {
                        const { tagName } = values;
                        try {
                            await client.models.Tags.update({
                                id: currenTageId,
                                tagName: tagName,
                            }, { authMode: "userPool" });
                ...
                <Button
                    onClick={async () => {
                        try {
                            await client.models.Tags.delete({
                                id: currenTageId,
                            }, { authMode: "userPool" });
                            feachTag(pageTokens[currentPageIndex]);
                            formEdit.resetFields();
